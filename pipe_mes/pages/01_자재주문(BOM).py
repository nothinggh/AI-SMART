...
if tab_choice == "1. 자재 주문 등록":
    ...
    conn = sqlite3.connect(DB_PATH)
    df_raw = pd.read_sql_query("SELECT * FROM BOM", conn)
    conn.close()
    ...
    filtered_rows = len(df)  # 검색/필터링된 행 수
    ...
    m3.metric(label="검색 항목 총 수량", value=f"{total_qty:,} 개")
    m4.metric(label="검색 항목 총 금액", value=f"{total_price_sum:,.0f} 원")
    ...
    if not df.empty:
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "weight": st.column_config.NumberColumn("중량(kg)"),
                "price": st.column_config.NumberColumn("가격(원)"),
